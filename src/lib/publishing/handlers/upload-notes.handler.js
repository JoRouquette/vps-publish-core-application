...
exports.UploadNotesHandler = void 0;
const _core_domain_1 = require("@core-domain");
const string_utils_1 = require("@core-domain/utils/string.utils");
class UploadNotesHandler {
    constructor(markdownRenderer, contentStorage, manifestStorage, logger, notesStorage, ignoredTags, noteHashService) {
    ...
    async handle(command) {
        ...
        if (this.notesStorage) {
            ...
            await this.updateManifestForSession(sessionId, [], notes, manifestStorage, logger, folderDisplayNames);
            logger?.debug('Upload batch persisted without rendering; finalization remains authoritative', {
                count: notes.length,
            });
            return {
                sessionId,
        ...
        for (let i = 0; i < notes.length; i += CONCURRENCY) {
            ...
            const batchResults = await Promise.allSettled(batch.map(async (note) => {
        ...
        if (succeeded.length > 0) {
            ...
            const pages = await Promise.all(succeeded.map(async (n) => {
                try {
                    ...
                    return {
                        ...
                        title: n.title,
                        route: n.routing.fullPath,
                        slug: _core_domain_1.Slug.from(n.routing.slug),
                    ...
                    throw new Error(`Invalid slug "${n.routing.slug}" for note "${n.vaultPath}": ${err instanceof Error ? err.message : 'Unknown error'}`);
                }
            }));
            ...
            await this.updateManifestForSession(sessionId, pages, succeeded, manifestStorage, logger, folderDisplayNames);
        }
    ...
    hasRouting(note) {
        const routing = 'routing' in note && typeof note.routing === 'object' && note.routing !== null
            ? note.routing
            : null;
        return (routing !== null &&
    ...
    buildRenderManifest(sessionId, existingManifest, notes) {
        const basePages = existingManifest && existingManifest.sessionId === sessionId ? existingManifest.pages : [];
    ...
    buildManifestPage(note) {
        return {
            id: note.noteId,
            title: note.title,
            route: note.routing.fullPath,
            slug: _core_domain_1.Slug.from(note.routing.slug),
    ...
    async updateManifestForSession(sessionId, newPages, notes, manifestStorage, logger, providedDisplayNames) {
        ...
        await manifestStorage.atomicUpdate(async (existing) => {
            ...
            manifest.pages = Array.from(byId.values()).sort((a, b) => b.publishedAt.getTime() - a.publishedAt.getTime());
    ...
    shouldRebuildIndexesAfterUpload() {
        return !this.notesStorage;
    }
    buildHtmlPage(note, bodyHtml) {
  ...
  <div class="markdown-body">
    ...
    renderFrontmatter(note) {
        ...
        for (const [k, v] of Object.entries(fm)) {
            if (!this.hasRenderableFrontmatterValue(v))
                continue;
        ...
        if (entries.length === 0)
            return '';
        const renderValue = (value, depth, path) => {
            if (!this.hasRenderableFrontmatterValue(value))
                return '';
            if (value === null || value === undefined)
                return '<span class="fm-value is-empty">""</span>';
            ...
            const renderedText = typeof value === 'string'
                ? this.renderFrontmatterText(value, note, path)
                : this.escapeHtml(String(value));
            return `<span class="fm-value">${renderedText}</span>`;
        };
        const renderEntry = (key, value, depth, path) => {
            if (!this.hasRenderableFrontmatterValue(value))
                return '';
            const label = (0, string_utils_1.humanizePropertyKey)(key) || key;
            return `
    ...
    hasRenderableFrontmatterValue(value) {
        if (value === null || value === undefined)
            return false;
        if (typeof value === 'string')
            return value.trim().length > 0;
        if (Array.isArray(value))
            return value.some((item) => this.hasRenderableFrontmatterValue(item));
        if (typeof value === 'object') {
            return Object.values(value).some((v) => this.hasRenderableFrontmatterValue(v));
        }
        return true;
    }
    resolveContentStorage(sessionId) {
        if (typeof this.contentStorage === 'function') {
            return this.contentStorage(sessionId);
        }
        return this.contentStorage;
    }
    resolveManifestStorage(sessionId) {
        if (typeof this.manifestStorage === 'function') {
            return this.manifestStorage(sessionId);
        }
        return this.manifestStorage;
    }
    renderFrontmatterText(text, note, path) {
        const tokensRegex = /(!?\[\[[^\]]+\]\])/g;
        const assets = (note.assets ?? []).filter((a) => (a.origin === 'frontmatter' || !a.origin) &&
            (!a.frontmatterPath || a.frontmatterPath === path));
        const wikilinks = (note.resolvedWikilinks ?? []).filter((l) => (l.origin === 'frontmatter' || !l.origin) &&
    ...
    renderFrontmatterAssetLink(raw, assets) {
    ...
    renderFrontmatterWikilink(raw, wikilinks) {
        const link = wikilinks.find((l) => l.raw === raw);
        if (!link) {
            return this.escapeHtml(raw);
        }
        const label = this.escapeHtml(link.alias ?? (link.subpath ? `${link.target}#${link.subpath}` : link.target));
        if (link.isResolved) {
            const hrefTarget = link.href ?? link.path ?? link.target;
            const href = this.escapeAttribute(encodeURI(hrefTarget));
            return `<a class="fm-link fm-wikilink wikilink" href="${href}" data-wikilink="${this.escapeAttribute(link.target)}">${label}</a>`;
        }
        return `<span class="fm-value fm-wikilink-unresolved wikilink wikilink-unresolved" role="link" aria-disabled="true" tabindex="0" title="${this.escapeAttribute(_core_domain_1.UNAVAILABLE_INTERNAL_PAGE_MESSAGE)}" data-tooltip="${this.escapeAttribute(_core_domain_1.UNAVAILABLE_INTERNAL_PAGE_MESSAGE)}" data-wikilink="${this.escapeAttribute(link.target)}">${label}</span>`;
    }
    extractEmbedTarget(raw) {
    ...
    extractSeoFields(note, logger) {
        ...
        try {
            ...
            const rawModified = flat['lastmodified'] ?? flat['lastmodifiedat'] ?? flat['updated'] ?? flat['modified'];
            if (rawModified) {
...
exports.UploadNotesHandler = UploadNotesHandler;
