...
exports.ComputeRoutingService = void 0;
class ComputeRoutingService {
    constructor(logger) {
    ...
    process(notes) {
        const routed = notes.map((note) => {
            ...
            if (segments.length === 0) {
                ...
                const sluggedDirs = note.folderConfig.flattenTree || isAdditionalFile
                    ? []
        ...
        for (const [folderId, folderNotes] of flattenedFolders.entries()) {
            ...
            for (const [route, conflictingNotes] of routeMap.entries()) {
                if (conflictingNotes.length > 1) {
                    ...
                    throw new Error(`Slug collision detected: ${conflictingNotes.length} notes map to the same route "${route}" in flattened folder. Conflicting files: ${paths}. Please rename one of the files or disable flattenTree for this folder.`);
                }
            }
        }
    }
}
exports.ComputeRoutingService = ComputeRoutingService;
