...
exports.ContentSanitizerService = void 0;
const string_utils_1 = require("@core-domain/utils/string.utils");
class ContentSanitizerService {
    constructor(vpsCleanupRules = [], frontmatterKeysToExclude, frontmatterTagsToExclude, logger) {
        this.vpsCleanupRules = vpsCleanupRules;
        this.frontmatterKeysToExclude = frontmatterKeysToExclude;
        this.frontmatterTagsToExclude = frontmatterTagsToExclude;
        this.logger = logger;
    }
    process(notes) {
    ...
    sanitizeFrontmatter(notes) {
        this.logger?.debug('Sanitizing frontmatter for notes', { notesLength: notes.length });
        const keysToRemove = new Set((this.frontmatterKeysToExclude || []).map((k) => (0, string_utils_1.normalizePropertyKey)(k)).filter(Boolean));
        ...
        for (const note of notes) {
            ...
            for (const key of Object.keys(note.frontmatter.flat)) {
                if (keysToRemove.has((0, string_utils_1.normalizePropertyKey)(key))) {
    ...
    sanitizeTags(notes) {
        ...
        for (const note of notes) {
            ...
            if (!Array.isArray(tags) || tags.length === 0)
                continue;
            const filtered = tags.filter((tag) => typeof tag !== 'string' || !exclude.includes(tag.toLowerCase()));
            if (filtered.length !== tags.length) {
                this.logger?.debug('Excluding frontmatter tags', {
                    noteId: note.noteId,
                    removed: tags.filter((tag) => typeof tag === 'string' && exclude.includes(tag.toLowerCase())),
                });
    ...
    removeNestedKeys(target, keysToRemove, noteId) {
        for (const key of Object.keys(target)) {
            const normalized = (0, string_utils_1.normalizePropertyKey)(key);
            ...
            if (value && typeof value === 'object' && !Array.isArray(value)) {
                this.removeNestedKeys(value, keysToRemove, noteId);
    ...
    deleteNestedPath(target, rawPath) {
        const segments = rawPath
            .split('.')
            .map((s) => (0, string_utils_1.normalizePropertyKey)(s))
            .filter(Boolean);
        if (segments.length === 0)
            return;
        this.deletePathRecursive(target, segments);
    }
    deletePathRecursive(target, segments) {
        if (segments.length === 0)
            return;
        const [head, ...rest] = segments;
        for (const key of Object.keys(target)) {
            if ((0, string_utils_1.normalizePropertyKey)(key) !== head)
                continue;
            if (rest.length === 0) {
                delete target[key];
                return;
            ...
            if (next && typeof next === 'object' && !Array.isArray(next)) {
                this.deletePathRecursive(next, rest);
...
exports.ContentSanitizerService = ContentSanitizerService;
